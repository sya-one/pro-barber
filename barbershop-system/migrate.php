 ...
 * Run this script via CLI: php migrate.php
 * Or via web (with proper security): migrate_web.php
 */

// Try different connection methods for XAMPP/cPanel compatibility
function getConnection() {
    ...
    $connections = [
        ...
        "mysql:host=localhost;dbname=$db_name;charset=utf8mb4;unix_socket=/tmp/mysql.sock",
    ];
...
// ============================================================
function createMigrationsTrackingTable($db) {
    try {
...
createMigrationsTrackingTable($db);
...
function markMigrationExecuted($db, $migration_name) {
    try {
        ...
        // Silent fail - migration might already be tracked
    }
}
...
$migrations = [
    ...
    'create_expense_categories_table' => function($db) {
        ...
        $categories = ['Rent', 'Electricity', 'Water', 'Salaries', 'Products', 'Equipment', 'Marketing', 'Internet', 'Maintenance', 'Other'];
        foreach ($categories as $cat) {
            $stmt = $db->prepare("INSERT IGNORE INTO expense_categories (name) VALUES (?)");
            $stmt->execute([$cat]);
        }
    },
    ...
    'update_gallery_table_schema' => function($db) {
         ...
         if (!$stmt->fetch()) {
             ...
             // If title exists but is empty, populate from caption
             $db->exec("UPDATE gallery SET title = caption WHERE title IS NULL OR title = ''");
         }
         // Migrate data: set status from is_active
         $stmt = $db->prepare("SELECT status FROM gallery LIMIT 1");
         $stmt->execute();
         $has_status_val = $stmt->fetchColumn();
         if ($has_status_val === false || ($has_status_val !== 'active' && $has_status_val !== 'inactive')) {
...
foreach ($migrations as $name => $callback) {
    ...
    if (isMigrationExecuted($db, $name)) {
        echo "○ $name - Skipped (already executed)\n";
        $skipped[] = $name;
        continue;
    }
    
    try {
        // Check if the change already exists (for ALTER TABLE migrations)
        if (strpos($name, 'add_') === 0 || strpos($name, 'add_') === 0) {
            // Try to run the migration anyway - it will fail if column exists
        }
        ...
        if (strpos($msg, 'Duplicate column') !== false || 
            ...
            // Mark as executed since the intended change exists
            markMigrationExecuted($db, $name);
            echo "○ $name - Skipped (already exists)\n";
            $skipped[] = $name;
        } else {
            echo "✗ $name - Error: $msg\n";
            $errors[] = ['name' => $name, 'error' => $msg];
        }
    }
}
...
echo "Skipped: " . count($skipped) . " migrations\n";
